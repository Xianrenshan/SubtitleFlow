...
def call_api_with_prompt(config: dict, prompt: str, system_prompt: Optional[str] = None,
    ...
    # 字幕翻译场景默认关闭思考；仅当用户显式设为 True 时才开启
...
def batch_translate_online(entries: List[Dict], config: dict,
    ...
    """
    api_cfg = config.get("online_api", {})
    batch_size = api_cfg.get("batch_size", 8)       # 🆕 默认 8（原 5）
    request_delay = api_cfg.get("request_delay", 0.2)
    time_window = api_cfg.get("time_window", 25.0)  # 🆕 时间窗口 25 秒（原 15）
    ...
    while i < total:
        ...
        # 更新上下文（最后 3 句的译文，🆕 从 2 增加到 3）
        context_prev = "\n".join(
            [f"[{k+1}] {zh_texts[k]}"
             for k in range(max(0, len(zh_texts) - 3), len(zh_texts))]
        )
